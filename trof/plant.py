...
def advance_plant(
    ...
    next_store = state.store_energy_kwh + dt * (action.store_charge_kw - action.store_discharge_kw - store_loss_kw)
    ...
    store_residual_kw = (next_store - state.store_energy_kwh) / dt - action.store_charge_kw + action.store_discharge_kw + store_loss_kw
    ...
    dhw_served_kw = min(float(realised["dhw_demand_kw"]), action.dhw_heat_kw + action.store_discharge_dhw_kw)
    process_served_kw = min(float(realised["process_demand_kw"]), action.process_heat_kw + action.store_discharge_process_kw)
    ...
    row = {
        ...
        "store_discharge_kw": action.store_discharge_kw,
